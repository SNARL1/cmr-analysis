...
The following code differs in several important respects from other xxxxx_analysis_mrmr.Rmd files in this directory. 
Specifically, due to the large size of the frog capture history matrix, the file is formatted for running on a high-performance computing server with 32 CPU cores (16 cores are used under the current specifications) and 256 GB of RAM.  
...
captures <- read_csv(here::here("data", "clean", "70550_capture.csv"))
surveys <- read_csv(here::here("data", "clean", "70550_survey.csv"))
translocations <- read_csv(here::here("data", "clean", "70550_translocation.csv"))
```
...
model_70550 <- fit_model(
                    ...
                    threads_per_chain = 4,
                    dir = "/home/snarg/roland/70550") # to prevent writing to a locked system directory 
```
...
```{r write_model}
write_rds(model_70550, here::here("out", "model", "70550_model.rds"), "xz", compression = 9L)
gc() # to free unused memory
```
```{r read_model}
# model_70550 <- read_rds(here::here("out", "model", "70550_model.rds"))
```
...
### Save custom plots
```{r save_plot}
ggsave(here::here("out", "plots", "70550_mrmr_plots.png"), width = 6.5, height = 7.2)
gc() # to free unused memory
```
...
survival_cohort %>% write_csv(here::here("out", "tables", "70550_survival_cohort.csv"))
survival_ind %>% write_csv(here::here("out", "tables", "70550_survival_ind.csv"))
```
