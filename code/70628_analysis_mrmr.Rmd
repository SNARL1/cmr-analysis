---
title: "70628 - Analyze capture-mark-recapture data using *mrmr*"
author: "Roland Knapp"
...
captures <- read_csv(here::here("data", "clean", "70628_capture.csv"))
surveys <- read_csv(here::here("data", "clean", "70628_survey.csv"))
translocations <- read_csv(here::here("data", "clean", "70628_translocation.csv"))
```

## Check datasets
```{r check_data}
data_70628 <- clean_data(captures = captures, surveys = surveys, translocations = translocations)
```

## Fit model
Add "adapt_delta = 0.99" or similar if divergent transitions occur. 
```{r fit_model}
model_70628 <- fit_model(
                    data = data_70628, 
                    chains = 4, 
                    parallel_chains = 4, 
...
```{r write_model}
write_rds(model_70628, here::here("out", "model", "70628_model.rds"), "xz", compression = 9L)
```
```{r read_model}
model_70628 <- read_rds(here::here("out", "model", "70628_model.rds"))
```
...
pars_to_plot <- c('alpha_lambda', 
                  ...
                  'sigma_phi', 
                  'beta_detect')
bayesplot::mcmc_trace(model_70628$m_fit$draws(pars_to_plot))

model_70628$m_fit$summary(pars_to_plot)
```
...
```{r model_visualize}
plot_model(model_70628, what = "abundance")
plot_model(model_70628, what = "recruitment")
plot_model(model_70628, what = "survival")
```

### Customize plots
```{r plots_custom, fig.height=8, fig.width=7}
library(ggplot2)
p1 <- plot_model(model_70628, what = "survival") 
p1 <- p1 + ggtitle("A. Survival of translocated frogs") +
    expand_limits(y = 0) +
    theme(legend.position = "none")
  
p2 <- plot_model(model_70628, what = "abundance") 
p2 <- p2 + ggtitle("B. Population size") +
    ylab("Number of adults") + 
    expand_limits(y = 0)
  
p3 <- plot_model(model_70628, what = "recruitment")
...
### Save custom plots
```{r save_plot}
ggsave(here::here("out", "plots", "70628_mrmr_plots.png"), width = 6.5, height = 7.2)
```

## Create survival tables
Only the cohort-level survival table is displayed (individual-level survival can be a large table).
```{r survival_tables}
survival_cohort <- survival_table(model_70628, by_cohort = TRUE)
survival_cohort

survival_ind <- survival_table(model_70628, by_individual = TRUE) 
```

### Save survival tables
```{r save_survival_tables}
survival_cohort %>% write_csv(here::here("out", "tables", "70628_survival_cohort.csv"))
survival_ind %>% write_csv(here::here("out", "tables", "70628_survival_ind.csv"))
```
