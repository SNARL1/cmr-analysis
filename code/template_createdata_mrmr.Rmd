...
Not all sites have received frog translocations.
Run or skip the following code chunks as necessary.
...
```{sql retrieve-translocation-data, connection=con, output.var = "translocation_70414"}
select 
  release_siteid1, 
  release_date, 
  type, 
...
translocation_70414_mrmr <- read_csv(here::here("data", "clean", "70414_translocation.csv")) %>% 
...
```{sql retrieve-survey-data, connection=con, output.var = "survey_70414"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
When multiple numbered sites exist in survey area, include only one survey for entire site on each survey date 70414 is the primary site at this location and was surveyed during each secondary period.
Because cmr and visual surveys exist for same date, keep cmr survey only.
Change survey_type "visual" to "cmr".
...
```{r add-translocations-as-surveys}
...
survey_70414_mrmr %>% 
  ...
  select(survey_date) %>% 
  inner_join(survey_70414_mrmr, by = "survey_date") %>% 
...
# survey_70414_mrmr <- read_csv(here::here("data", "clean", "70414_survey.csv"))
```
...
```{sql retrieve-capture-data, connection=con, output.var = "capture_70414"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
where site_id in (70414, 70688) and
      survey_type = 'cmr'
```
...
# Simpler alternative
setdiff(capture_70414$visit_date, survey_70414_mrmr$survey_date) %>% as.Date(origin = "1970-01-01")
...
# Simpler alternative
setdiff(survey_70414_mrmr$survey_date, capture_70414$visit_date) %>% as.Date(origin = "1970-01-01")
```
...
# capture_70414_mrmr <- read_csv(here::here("data", "clean", "70414_capture.csv")) %>% 
#    mutate(pit_tag_id = as.character(pit_tag_id))
```
