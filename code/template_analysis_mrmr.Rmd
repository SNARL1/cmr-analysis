---
title: "xxxxx - Analyze capture-mark-recapture data using *mrmr*"
author: "Roland Knapp"
...
captures <- read_csv(here::here("data", "clean", "xxxxx_capture.csv"))
surveys <- read_csv(here::here("data", "clean", "xxxxx_survey.csv"))
translocations <- read_csv(here::here("data", "clean", "xxxxx_translocation.csv"))
```

## Check datasets

```{r check-data}
data_xxxxx <- clean_data(captures = captures, surveys = surveys, translocations = translocations)
```
...
- To read model file, use "model_xxxxx <- read_rds(here::here("out", "model", "xxxxx_model.rds"))"
...
pars_to_plot <- c('alpha_lambda', 
                  ...
                  'sigma_phi', 
                  'beta_detect')
bayesplot::mcmc_trace(model_xxxxx$m_fit$draws(pars_to_plot))

model_xxxxx$m_fit$summary(pars_to_plot)
```
...
```{r plots-default}
# plot_model(model_xxxxx, what = "abundance")
# plot_model(model_xxxxx, what = "recruitment")
# plot_model(model_xxxxx, what = "survival")
```
...
library(ggplot2)
p1 <- plot_model(model_xxxxx, what = "survival") 
p1 <- p1 + ggtitle("A. Survival of translocated frogs") +
    expand_limits(y = 0) +
    theme(legend.position = "none")
  
p2 <- plot_model(model_xxxxx, what = "abundance") 
p2 <- p2 + ggtitle("B. Population size") +
    ylab("Number of adults") + 
    expand_limits(y = 0)
  
p3 <- plot_model(model_xxxxx, what = "recruitment")
...
```{r save-plot}
ggsave(here::here("out", "plots", "xxxxx_mrmr_plots.png"), width = 6.5, height = 7.2)
```
...
```{r survival-tables}
survival_cohort <- survival_table(model_xxxxx, by_cohort = TRUE)
survival_cohort

survival_ind <- survival_table(model_xxxxx, by_individual = TRUE) 
```

### Save survival tables

```{r save-survival-tables}
survival_cohort %>% write_csv(here::here("out", "tables", "xxxxx_survival_cohort.csv"))
survival_ind %>% write_csv(here::here("out", "tables", "xxxxx_survival_ind.csv"))
```
