---
title: "70370 - Create datasets for analysis using *mrmr*"
author: "Roland Knapp"
...
```{sql retrieve_translocation_data, connection=con, output.var = "translocation_70370"}
select 
  release_siteid1, 
  release_date, 
  type, 
...
where release_siteid1 = 70370 and 
      type = 'translocation'
```
```{r}
dbDisconnect(con)
...
translocation_70370 %>% write_csv(here::here("data", "raw", "70370_translocation_raw.csv"))
```
```{r read_raw_translocation_data, message=FALSE}
translocation_70370 <- read_csv(here::here("data", "raw", "70370_translocation_raw.csv")) %>% 
...
assert_that(noNA(translocation_70370$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters (or 9 for early sites)
assert_that(!any(nchar(translocation_70370$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(translocation_70370$pit_tag_ref, "\\.1$")))

# Check for records of subadult frogs
translocation_70370 %>% drop_na(length) %>% filter(length < 40)

# Check for frogs that died during translocation
translocation_70370 %>% filter(grepl("dead", comment_relocate_frog) | grepl("died", comment_relocate_frog))

# Check for duplicate pit tags on each date
translocation_70370 %>% count(release_date, pit_tag_ref) %>% filter(n > 1)

# Are release dates correct?
translocation_70370 %>% distinct(release_date) %>% glimpse()
```

### Clean translocation data
For 2016 translocation, include only the 50 frogs collected from 72996 (on 2016-07-30) and not the 4 frogs collected from 74335. This simplifies the description and the 4 frogs from 74335 are inconsequential. 
```{r clean_translocation_data}
# Remove 4 frogs collected from 74335
translocation_70370 <- translocation_70370 %>% 
  filter(release_date != "2016-07-27")
```

### Create final translocation dataset formatted for mrmr
```{r final_translocation_data}
translocation_70370_mrmr <- translocation_70370 %>% 
...
translocation_70370_mrmr %>% write_csv(here::here("data", "clean", "70370_translocation.csv"))
```
```{r read_cleaned_translocation_data, message=FALSE}
translocation_70370 <- read_csv(here::here("data", "clean", "70370_translocation.csv")) %>% 
...
```{sql retrieve_survey_data, connection=con, output.var = "survey_70370"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
survey_70370_mrmr <- translocation_70370_mrmr %>% 
  distinct(site_id, release_date, type) %>% 
  rename(survey_type = type, survey_date = release_date) %>% bind_rows(survey_70370) %>% 
  arrange(survey_date) 
```
...
survey_70370_mrmr %>% count(site_id, survey_date) %>% filter(n > 1)

# Check for sites that were unsuitable for survey
survey_70370_mrmr %>% filter(visit_status != "suitable")
```
...
survey_70370_mrmr <- survey_70370_mrmr %>%
  select(site_id, survey_date, survey_type) %>% 
  mutate(primary_period = case_when(survey_date == "2016-07-30" ~ 1, 
                                    between(survey_date, ymd("2016-09-05"), ymd("2016-09-07")) ~ 2,
                                    between(survey_date, ymd("2017-07-26"), ymd("2017-07-27")) ~ 3,
                                    between(survey_date, ymd("2017-09-05"), ymd("2017-09-06")) ~ 4,
                                    survey_date == "2018-07-04" ~ 5, 
                                    between(survey_date, ymd("2018-08-01"), ymd("2018-08-03")) ~ 6,
                                    between(survey_date, ymd("2019-07-16"), ymd("2019-07-17")) ~ 7,
                                    survey_date == "2020-06-24" ~ 8)) 
...
survey_70370_mrmr %>% write_csv(here::here("data", "clean", "70370_survey.csv"))
```
```{r read_cleaned_survey_data}
survey_70370 <- read_csv(here::here("data", "clean", "70370_survey.csv"))
```
...
```{sql retrieve_capture_data, connection=con, output.var = "capture_70370"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
capture_70370 %>% write_csv(here::here("data", "raw", "70370_capture_raw.csv"))
```
```{r read_raw_capture_data, message=FALSE}
capture_70370 <- read_csv(here::here("data", "raw", "70370_capture_raw.csv")) %>% 
  mutate(pit_tag_ref = as.character(pit_tag_ref))
```
...
# Check for records where pit_tag_ref is NULL
assert_that(noNA(capture_70370$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters (or 9 for early sites)
assert_that(!any(nchar(capture_70370$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(capture_70370$pit_tag_ref, "\\.1$")))

# Check for records where species != ramu
capture_70370 %>% drop_na(species) %>% filter(species != "ramu")

# Check for records were survey_type = swab
assert_that(!any(capture_70370$survey_type == "swab"))

# Check for records where capture_animal_state is NULL
assert_that(noNA(capture_70370$capture_animal_state))

# Check for records where capture_animal_state == "dead"
assert_that(!any(capture_70370$capture_animal_state == "dead"))

# Check for records of subadult frogs
capture_70370 %>% drop_na(length) %>% filter(length < 40)

# Check for sites that were unsuitable for survey
assert_that(!any(capture_70370$visit_status != "suitable"))

# Check for duplicate capture records on each date
capture_70370 %>% count(visit_date, pit_tag_ref) %>% filter(n > 1)

# Check for captures that lack associated surveys
survey_70370 %>% 
  select(survey_date) %>% 
  rename(visit_date = survey_date) %>% 
  right_join(capture_70370, by = c("visit_date")) %>% 
...
survey_70370 %>% 
  select(survey_date) %>% 
  rename(visit_date = survey_date) %>% 
  left_join(capture_70370, by = c("visit_date")) %>% 
  filter(is.na(site_id))
```

### Clean capture data
```{r clean_capture_data}
# Remove dead frogs and 4 frogs translocated from 74335
capture_70370 <- capture_70370 %>% 
  filter(capture_animal_state != "dead" &
         !pit_tag_ref %in% c('900067000114101', '900067000114116', '900067000114146', '900067000114175'))
```

### Create final capture dataset formatted for mrmr
```{r final_capture_data}
capture_70370_mrmr <- capture_70370 %>%
...
capture_70370_mrmr %>% write_csv(here::here("data", "clean", "70370_capture.csv"))
```
```{r read_cleaned_capture_data}
capture_70370 <- read_csv(here::here("data", "clean", "70370_capture.csv"))
```
