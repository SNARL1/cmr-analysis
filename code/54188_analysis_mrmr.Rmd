---
title: "54188 - Analyze capture-mark-recapture data using *mrmr*"
author: "Roland Knapp"
output: html_notebook
editor_options: 
  chunk_output_type: inline
---
This notebook analyzes capture-mark-recapture data using the *mrmr* package. The code creates the required survey, capture, and translocation data frames from saved CSV files (created by 54188_createdata_mrmr.Rmd), checks the datasets, fits the model, evaluates potential non-convergence of the MCMC algorithm, and visualizes model results. 
...
captures <- read_csv(here::here("data", "clean", "54188_capture.csv"))
surveys <- read_csv(here::here("data", "clean", "54188_survey.csv"))
# translocations <- read_csv(here::here("data", "clean", "54188_translocation.csv"))
```
...
```{r check-data}
data_54188 <- clean_data(captures = captures, surveys = surveys)
```
...
```{r write-model}
write_rds(model_54188, here::here("out", "model", "54188_model.rds"), "xz", compression = 9L)

# reload the model if you've been away:
# model_54188 <- read_rds(here::here("out", "model", "54188_model.rds"))
```

- To read model file, use "model_54188 <- read_rds(here::here("out", "model", "54188_model.rds"))"
...
pars_to_plot <- c('alpha_lambda', 
                  ...
                  'sigma_phi', 
                  'beta_detect')
bayesplot::mcmc_trace(model_54188$m_fit$draws(pars_to_plot))

model_54188$m_fit$summary(pars_to_plot)
```
...
```{r plots-default}
# plot_model(model_54188, what = "abundance")
# plot_model(model_54188, what = "recruitment")
# plot_model(model_54188, what = "survival")
```
...
library(ggplot2)
p2 <- plot_model(model_54188, what = "abundance") 
p2 <- p2 + ggtitle("A. Population size") +
    ylab("Number of adults") + 
    expand_limits(y = 0)
  
p3 <- plot_model(model_54188, what = "recruitment")
p3 <- p3 + ggtitle("B. Recruitment") +
    ylab("Number of adults")
...
```{r save-plot}
ggsave(here::here("out", "plots", "54188_mrmr_plots.png"), width = 6.5, height = 7.2)
```
...
```{r survival-tables}
survival_cohort <- survival_table(model_54188, by_cohort = TRUE)
survival_cohort

survival_ind <- survival_table(model_54188, by_individual = TRUE) 
```

### Save survival tables

```{r save-survival-tables}
survival_cohort %>% write_csv(here::here("out", "tables", "54188_survival_cohort.csv"))
survival_ind %>% write_csv(here::here("out", "tables", "54188_survival_ind.csv"))
```
