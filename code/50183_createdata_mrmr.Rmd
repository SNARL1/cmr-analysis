...
visit_50183 <- read_csv(here::here("data", "raw", "current_year", "survey_50183.csv")) %>% 
  select(record_id, site_id, visit_date, visit_status, visit_comment) %>% 
  rename(comment_visit = visit_comment)

survey_50183 <- read_csv(here::here("data", "raw", "current_year", "survey_description_50183.csv")) %>% 
...
amphibian_capture_50183 <- read_csv(here::here("data", "raw", "current_year", "survey_amphibian_capture_50183.csv"), col_types = cols(.default = "?", species="c", capture_life_stage = "c" ) ) %>% 
  filter(method == "cmr") %>% 
...
survey_append_50183 <- visit_50183 %>% 
  inner_join(survey_50183, by = c("record_id")) %>% 
  select(-record_id)

capture_append_50183 <- visit_50183 %>% 
  inner_join(survey_50183, by = c("record_id")) %>% 
  inner_join(amphibian_capture_50183, by = c("record_id")) %>% 
  select(-record_id)


### append current year reintro
relocate_50183 <- read_csv(here::here("data", "raw", "current_year", "relocate_50183.csv"))
relocate_amphibians_50183 <- read_csv(here::here("data", "raw", "current_year", "relocate_amphibians_50183.csv")) %>% 
  mutate(pit_tag_ref = as.character(pit_tag_ref))
  

relocate_50183_2024 <- relocate_50183 %>% 
  inner_join(relocate_amphibians_50183, by = c("record_id" = "record_id")) %>% 
  mutate(pit_tag_ref = as.character(pit_tag_ref)) %>% 
  select(release_location, release_date, pit_tag_ref, tag_new, sex, length, weight, swab_id, relocate_comments) 
...
reintroduction_50183 <- relocate_50183_2024 %>% 
...
survey_50183 <- reintroduction_50183 %>% 
  ...
  select(-survey_date_2)
# revise the 2024 reintroduction to be +1 day to not overlap with the CMR; it occurred in the evening after the day's survey was complete.
```
...
# append 2023 and current year data
...
# for Marmot, there were two CMR surveys both in 2023 that were known to have zero observed frogs.

# Simpler alternative
setdiff(survey_50183$survey_date, capture_50183$visit_date) %>% as.Date(origin = "1970-01-01") # this includes reintro survey visit dates, which have no captures
```
