---
title: "50785 - Analyze capture-mark-recapture data using *mrmr*"
author: "Roland Knapp"
...
captures <- read_csv(here::here("data", "clean", "50785_capture.csv"))
surveys <- read_csv(here::here("data", "clean", "50785_survey.csv"))
translocations <- read_csv(here::here("data", "clean", "50785_translocation.csv"))
```

## Check datasets
```{r check-data}
data_50785 <- clean_data(captures = captures, surveys = surveys, translocations = translocations)
```
...
```{r write-model}
write_rds(model_50785, here::here("out", "model", "50785_model.rds"), "xz", compression = 9L)
```
```{r read-model}
#model_50785 <- read_rds(here::here("out", "model", "50785_model.rds"))
```
...
pars_to_plot <- c('alpha_lambda', 
                  ...
                  'sigma_phi', 
                  'beta_detect')
bayesplot::mcmc_trace(model_50785$m_fit$draws(pars_to_plot))

model_50785$m_fit$summary(pars_to_plot)
```
...
```{r plots-default}
# plot_model(model_50785, what = "abundance")
# plot_model(model_50785, what = "recruitment")
# plot_model(model_50785, what = "survival")
```

### Customize plots
```{r plots-custom, fig.height=8, fig.width=7}
library(ggplot2)
p1 <- plot_model(model_50785, what = "survival") 
p1 <- p1 + ggtitle("A. Survival of translocated frogs") +
    expand_limits(y = 0) +
    theme(legend.position = "none")
  
p2 <- plot_model(model_50785, what = "abundance") 
p2 <- p2 + ggtitle("B. Population size") +
    ylab("Number of adults") + 
    expand_limits(y = 0)
  
p3 <- plot_model(model_50785, what = "recruitment")
...
### Save custom plots
```{r save-plot}
ggsave(here::here("out", "plots", "50785_mrmr_plots.png"), width = 6.5, height = 7.2)
```

## Create survival tables
Only the cohort-level survival table is displayed (individual-level survival can be a large table).
```{r survival-tables}
survival_cohort <- survival_table(model_50785, by_cohort = TRUE)
survival_cohort

survival_ind <- survival_table(model_50785, by_individual = TRUE) 
```

### Save survival tables
```{r save-survival-tables}
survival_cohort %>% write_csv(here::here("out", "tables", "50785_survival_cohort.csv"))
survival_ind %>% write_csv(here::here("out", "tables", "50785_survival_ind.csv"))
```
