---
title: "74976 - Create datasets for analysis using *mrmr*"
author: "Roland Knapp"
...
```{sql retrieve_translocation_data, connection=con, output.var = "translocation_74976"}
select 
  release_siteid1, 
  release_date, 
  type, 
...
where release_siteid1 = 74976 and
      type = 'translocation'
```
```{r}
dbDisconnect(con)
...
translocation_74976 %>% write_csv(here::here("data", "raw", "74976_translocation_raw.csv"))
```
```{r read_raw_translocation_data, message=FALSE}
translocation_74976 <- read_csv(here::here("data", "raw", "74976_translocation_raw.csv")) %>% 
  mutate(pit_tag_ref = as.character(pit_tag_ref))
```

### Retain only relevant columns
```{r translocation_relevant_columns}
translocation_74976 <- translocation_74976 %>% 
...
assert_that(noNA(translocation_74976$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters
assert_that(!any(nchar(translocation_74976$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(translocation_74976$pit_tag_ref, "\\.1$")))

# Check for records of subadult frogs
translocation_74976 %>% drop_na(length) %>% filter(length < 40)

# Check for duplicate pit tags on each date
translocation_74976 %>% count(release_date, pit_tag_ref) %>% filter(n > 1)

# Are release dates correct?
translocation_74976 %>% distinct(release_date) %>% glimpse()
```
...
translocation_74976_mrmr <- translocation_74976 %>% 
...
translocation_74976_mrmr %>% write_csv(here::here("data", "clean", "74976_translocation.csv"))
```
```{r read_cleaned_translocation_data, message=FALSE}
translocation_74976 <- read_csv(here::here("data", "clean", "74976_translocation.csv")) %>% 
...
```{sql retrieve_survey_data, connection=con, output.var = "survey_74976"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
survey_74976 %>% write_csv(here::here("data", "raw", "74976_survey_raw.csv"))
```
```{r read_raw_survey_data, message=FALSE}
survey_74976 <- read_csv(here::here("data", "raw", "74976_survey_raw.csv"))
```

### Include only one survey for entire site on each survey date
74976 is the primary site and was surveyed during each secondary period. 72973 was only surveyed in recent years following frog dispersal to the site. 
```{r}
survey_74976 <- survey_74976 %>% 
  filter(site_id == 74976) %>%
...
survey_74976_mrmr <- translocation_74976 %>% 
  distinct(site_id, release_date, type) %>% 
  rename(survey_type = type, survey_date = release_date) %>% bind_rows(survey_74976) %>% 
  arrange(survey_date) 
```
...
survey_74976_mrmr %>% count(site_id, survey_date) %>% filter(n > 1)

# Check for sites that were unsuitable for survey
survey_74976_mrmr %>% filter(visit_status != "suitable")
```
...
survey_74976_mrmr <- survey_74976_mrmr %>%
  select(site_id, survey_date, survey_type) %>% 
  mutate(primary_period = case_when(survey_date == "2018-07-18" ~ 1, 
                                    between(survey_date, ymd("2018-08-17"), ymd("2018-08-19")) ~ 2,
                                    between(survey_date, ymd("2019-07-31"), ymd("2019-08-02")) ~ 3,
                                    between(survey_date, ymd("2020-07-01"), ymd("2020-07-03")) ~ 4,
                                    survey_date == "2020-07-06" ~ 5, 
                                    between(survey_date, ymd("2020-08-20"), ymd("2020-08-22")) ~ 6,
                                    between(survey_date, ymd("2021-06-25"), ymd("2021-06-27")) ~ 7,
                                    between(survey_date, ymd("2021-08-18"), ymd("2021-08-20")) ~ 8)) 
...
survey_74976_mrmr %>% write_csv(here::here("data", "clean", "74976_survey.csv"))
```
```{r read_cleaned_survey_data}
survey_74976 <- read_csv(here::here("data", "clean", "74976_survey.csv"))
```
...
```{sql retrieve_capture_data, connection=con, output.var = "capture_74976"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
capture_74976 %>% write_csv(here::here("data", "raw", "74976_capture_raw.csv"))
```
```{r read_raw_capture_data, message=FALSE}
capture_74976 <- read_csv(here::here("data", "raw", "74976_capture_raw.csv")) %>% 
  mutate(pit_tag_ref = as.character(pit_tag_ref))
```
...
# Check for records where pit_tag_ref is NULL
assert_that(noNA(capture_74976$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters
assert_that(!any(nchar(capture_74976$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(capture_74976$pit_tag_ref, "\\.1$")))

# Check for records where species != ramu
capture_74976 %>% drop_na(species) %>% filter(species != "ramu")

# Check for records were survey_type = swab
assert_that(!any(capture_74976$survey_type == "swab"))

# Check for records where capture_animal_state is NULL
assert_that(noNA(capture_74976$capture_animal_state))

# Check for records where capture_animal_state == "dead"
assert_that(!any(capture_74976$capture_animal_state == "dead"))

# Check for records of subadult frogs
capture_74976 %>% drop_na(length) %>% filter(length < 40)

# Check for sites that were unsuitable for survey
assert_that(!any(capture_74976$visit_status != "suitable"))

# Check for duplicate capture records on each date
capture_74976 %>% count(visit_date, pit_tag_ref) %>% filter(n > 1)

# Check for captures that lack associated surveys
survey_74976 %>% 
  select(survey_date) %>% 
  rename(visit_date = survey_date) %>% 
  right_join(capture_74976, by = c("visit_date")) %>% 
...
capture_74976_mrmr <- capture_74976 %>%
...
capture_74976_mrmr %>% write_csv(here::here("data", "clean", "74976_capture.csv"))
```
```{r read_cleaned_capture_data}
capture_74976 <- read_csv(here::here("data", "clean", "74976_capture.csv"))
```
