  ---
title: "An introduction to mrmr: Tyndall"
...
```{r show-capture-data}
captures <- "../data/captures.tyndallpond.mrmr.csv" %>%
  read_csv
...
```{r show-trans-data}
translocations <- "../data/translocations.tyndallpond.mrmr.csv" %>%
  read_csv
...
```{r show-survey-data}
surveys <- "../data/surveys.tyndallpond.mrmr.csv" %>%
  read_csv
...
write_rds(mod, "../out/model_tyndall_mrmr.rds", "xz", compression = 9L)
```
...
```{r plot-abundance}
plot_model(mod, what = 'abundance') %>% ggsave
```
...
## Export plot
```{r}
ggsave("../out/tyndall_plots2.png", width = 6.5, height = 4.5)
```
...
ggsave("../out/tyndall_plot_all.png", width = 6.5, height = 7.2)
```
