---
title: "70619 - Create datasets for analysis using *mrmr*"
author: "Roland Knapp"
...
```{sql retrieve_translocation_data, connection=con, output.var = "translocation_70619"}
select 
  release_siteid1, 
  release_date, 
  type, 
...
where release_siteid1 = 70619 and
      type = 'translocation'
```
```{r}
dbDisconnect(con)
...
translocation_70619 %>% write_csv(here::here("data", "raw", "70619_translocation_raw.csv"))
```
```{r read_raw_translocation_data, message=FALSE}
translocation_70619 <- read_csv(here::here("data", "raw", "70619_translocation_raw.csv")) %>% 
...
assert_that(noNA(translocation_70619$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters (or 9 for early sites)
assert_that(!any(nchar(translocation_70619$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(translocation_70619$pit_tag_ref, "\\.1$")))

# Check for records of subadult frogs
translocation_70619 %>% drop_na(length) %>% filter(length < 40)

# Check for frogs that died during translocation
translocation_70619 %>% filter(grepl("dead", comment_relocate_frog) | grepl("died", comment_relocate_frog))

# Check for duplicate pit tags on each date
translocation_70619 %>% count(release_date, pit_tag_ref) %>% filter(n > 1)

# Are release dates correct?
translocation_70619 %>% distinct(release_date) %>% glimpse()
```
...
translocation_70619_mrmr <- translocation_70619 %>% 
...
translocation_70619_mrmr %>% write_csv(here::here("data", "clean", "70619_translocation.csv"))
```
```{r read_cleaned_translocation_data, message=FALSE}
translocation_70619 <- read_csv(here::here("data", "clean", "70619_translocation.csv")) %>% 
...
If surveys at which no frogs were captured are classified as "ves", include in "where" statement
```{sql retrieve_survey_data, connection=con, output.var = "survey_70619"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
survey_70619 %>% write_csv(here::here("data", "raw", "70619_survey_raw.csv"))
```
```{r read_raw_survey_data, message=FALSE}
survey_70619 <- read_csv(here::here("data", "raw", "70619_survey_raw.csv"))
```

### When multiple numbered sites exist in survey area, include only one survey for entire site on each survey date
70619 is the primary site at this location and was surveyed during each secondary period. 
When no adults were captured, surveys recorded as visual. For those, checked to see whether PIT tag was recorded. If not, removed survey. Change survey_type "visual" to "cmr". Restrict date range to include only relevant surveys. Opted to exclude surveys conducted after 2013 due to lack of surveys in most years. 
```{r include_relevant_surveys}
# When both ves and cmr surveys are to be included
survey_70619 <- survey_70619 %>% 
  add_count(visit_date) %>% 
  filter((n > 1 & survey_type == "cmr" | n == 1),
         (visit_date != "2009-09-18" & visit_date != "2011-08-23"),
         (visit_date > "2008-07-15" & visit_date <= "2013-08-15")) %>% 
...
survey_70619_mrmr <- translocation_70619_mrmr %>% 
  distinct(site_id, release_date, type) %>% 
  rename(survey_type = type, survey_date = release_date) %>% bind_rows(survey_70619) %>% 
  arrange(survey_date) 
```
...
survey_70619_mrmr %>% count(site_id, survey_date) %>% filter(n > 1)

# Check for sites that were unsuitable for survey
survey_70619_mrmr %>% filter(visit_status != "suitable")
```
...
survey_70619_mrmr <- survey_70619_mrmr %>%
...
survey_70619_mrmr %>% write_csv(here::here("data", "clean", "70619_survey.csv"))
```
```{r read_cleaned_survey_data}
survey_70619 <- read_csv(here::here("data", "clean", "70619_survey.csv"))
```
...
```{sql retrieve_capture_data, connection=con, output.var = "capture_70619"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
capture_70619 %>% write_csv(here::here("data", "raw", "70619_capture_raw.csv"))
```
```{r read_raw_capture_data, message=FALSE}
capture_70619 <- read_csv(here::here("data", "raw", "70619_capture_raw.csv")) %>% 
  mutate(pit_tag_ref = as.character(pit_tag_ref))
```
...
### Retain only relevant columns and visit dates
```{r capture_relevant_columns}
capture_70619 <- capture_70619 %>% 
  filter(visit_date > "2008-07-15" & visit_date <= "2013-08-15") %>% 
...
# Check for records where pit_tag_ref is NULL
assert_that(noNA(capture_70619$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters (or 9 for early sites)
assert_that(!any(nchar(capture_70619$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(capture_70619$pit_tag_ref, "\\.1$")))

# Check for records where species != ramu
capture_70619 %>% drop_na(species) %>% filter(species != "ramu")

# Check for records were survey_type = swab
assert_that(!any(capture_70619$survey_type == "swab"))

# Check for records where capture_animal_state is NULL
assert_that(noNA(capture_70619$capture_animal_state))

# Check for records where capture_animal_state == "dead"
assert_that(!any(capture_70619$capture_animal_state == "dead"))

# Check for records of subadult frogs
capture_70619 %>% drop_na(length) %>% filter(length < 40)

# Check for sites that were unsuitable for survey
assert_that(!any(capture_70619$visit_status != "suitable"))

# Check for duplicate capture records on each date
capture_70619 %>% count(visit_date, pit_tag_ref) %>% filter(n > 1)

# Check for captures that lack associated surveys
survey_70619 %>% 
  select(survey_date) %>% 
  rename(visit_date = survey_date) %>% 
  right_join(capture_70619, by = c("visit_date")) %>% 
...
survey_70619 %>% 
  select(survey_date) %>% 
  rename(visit_date = survey_date) %>% 
  left_join(capture_70619, by = c("visit_date")) %>% 
...
capture_70619_mrmr <- capture_70619 %>%
...
capture_70619_mrmr %>% write_csv(here::here("data", "clean", "70619_capture.csv"))
```
```{r read_cleaned_capture_data}
capture_70619 <- read_csv(here::here("data", "clean", "70619_capture.csv"))
```
