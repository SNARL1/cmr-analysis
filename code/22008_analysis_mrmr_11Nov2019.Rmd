...
---
Analysis of Tyndall Pond CMR dataset
...
Set working directory for Tyndall Pond
```{r}
...
```{r}
captures <- "captures.tyndallpond.mrmr.csv" %>% read_csv
translocations <- "translocations.tyndallpond.mrmr.csv" %>% read_csv
surveys <- "surveys.tyndallpond.mrmr.csv" %>% read_csv
```
...
model_tyndallpond <- fit_model(data_tyndallpond, cores = parallel::detectCores())
```
...
```{r}
post <- rstan::extract(model_tyndallpond$m_fit)

# Check that all translocation treatments match
t1 <- data_tyndallpond$survival_covariate_df %>%
...
survival_df <- post$phi %>%
  ...
  as_tibble %>%
  mutate(pit_tag_id = data_tyndallpond$survival_covariate_df$pit_tag_id[individual]) %>%
  left_join(data_tyndallpond$survival_covariate_df) %>%
  distinct(iter, primary_period, value, treatment) %>%
  left_join(data_tyndallpond$surveys %>% filter(secondary_period < 2))
...
```{r}
primary_period_dates <- model_tyndallpond$data$surveys %>%
  group_by(.data$primary_period) %>%
  summarize(date = min(.data$survey_date),
            year = min(.data$year)) %>%
  ungroup

model_tyndallpond$post$s %>%
  reshape2::melt(varnames = c('iter', 'i', 'primary_period')) %>%
  as_tibble %>%
  mutate(pit_tag_id = dimnames(model_tyndallpond$data$stan_d$Y)[[1]][.data$i]) %>%
  filter(.data$pit_tag_id %in% as.character(model_tyndallpond$data$translocations$pit_tag_id)) %>%
  left_join(distinct(model_tyndallpond$data$translocations,
                     .data$pit_tag_id, .data$release_date)) %>%
  left_join(model_tyndallpond$data$survival_covariate_df) %>%
