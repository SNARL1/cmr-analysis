...
```{sql retrieve_translocation_data, connection=con, output.var = "reintroduction_10109"}
select 
  release_siteid1, 
  release_date, 
  type, 
...
where release_siteid1 in (10109, 10110, 11896) and
      type = 'reintroduction'  
```
...
- Read saved files with (for example): translocation_10109 <- read_csv(here::here("data", "raw", "10109_translocation_raw.csv")) %>% 
...
translocation_10109 <- reintroduction_10109 %>% 
...
assert_that(noNA(translocation_10109$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters
assert_that(!any(nchar(translocation_10109$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(translocation_10109$pit_tag_ref, "\\.1$")))

# Check for records of subadult frogs
translocation_10109 %>% drop_na(length) %>% filter(length < 40)

# Check for duplicate pit tags on each date
translocation_10109 %>% count(release_date, pit_tag_ref) %>% filter(n > 1)

# Are release dates correct?
translocation_10109 %>% distinct(release_date) %>% glimpse()
```
...
translocation_10109 <- translocation_10109 %>% 
...
translocation_10109 %>% write_csv(here::here("data", "clean", "10109_translocation.csv"))
```
...
survey_10109 <- translocation_10109 %>% 
  distinct(site_id, release_date, type) %>% 
  rename(survey_type = type, survey_date = release_date) %>% bind_rows(survey_10109) %>% 
  arrange(survey_date) 
```
...
survey_10109 %>% count(site_id, survey_date) %>% filter(n > 1)

# Check for sites that were unsuitable for survey
survey_10109 %>% filter(visit_status != "suitable")
```
...
survey_10109 <- survey_10109 %>%
  ...
  mutate(primary_period = case_when(survey_date == "2021-07-28" ~ 1,
...
# Check for records where pit_tag_ref is NULL
assert_that(noNA(capture_10109$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters (or 9 for early sites)
assert_that(!any(nchar(capture_10109$pit_tag_ref) != 15))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(capture_10109$pit_tag_ref, "\\.1$")))

# Check for records where species != ramu
capture_10109 %>% drop_na(species) %>% filter(species != "ramu")

# Check for records were survey_type = swab
assert_that(!any(capture_10109$survey_type == "swab"))

# Check for records where capture_animal_state is NULL
assert_that(noNA(capture_10109$capture_animal_state))

# Check for records where capture_animal_state == "dead"
assert_that(!any(capture_10109$capture_animal_state == "dead"))
### resolved by removing dead frogs in next chunk

# Check for records of subadult frogs
capture_10109 %>% drop_na(length) %>% filter(length < 40)

# Check for sites that were unsuitable for survey
assert_that(!any(capture_10109$visit_status != "suitable"))

# Check for duplicate capture records on each date
capture_10109 %>% count(visit_date, pit_tag_ref) %>% filter(n > 1)

# Check for captures that lack associated surveys
survey_10109 %>% 
  select(survey_date, primary_period) %>% 
  rename(visit_date = survey_date) %>% 
  right_join(capture_10109, by = c("visit_date")) %>% 
  filter(is.na(primary_period))

# Simpler alternative
setdiff(capture_10109$visit_date, survey_10109$survey_date) %>% as.Date(origin = "1970-01-01")
...
survey_10109 %>% 
  ...
  select(survey_date, primary_period) %>% 
  rename(visit_date = survey_date) %>% 
  left_join(capture_10109, by = c("visit_date")) %>% 
