---
title: "10114 - Analyze capture-mark-recapture data using *mrmr*"
author: "Roland Knapp"
output: html_notebook
editor_options: 
  chunk_output_type: inline
---
## CMR analysis - Goddard 10114

This notebook analyzes capture-mark-recapture data using the *mrmr* package. The code creates the required survey, capture, and translocation data frames from saved CSV files (created by 10114_createdata_mrmr.Rmd), checks the datasets, fits the model, evaluates potential non-convergence of the MCMC algorithm, and visualizes model results. 
...
captures <- read_csv(here::here("data", "clean", "10114_capture.csv"))
surveys <- read_csv(here::here("data", "clean", "10114_survey.csv"))
translocations <- read_csv(here::here("data", "clean", "10114_reintroduction.csv")) 
```

## Check datasets

```{r check-data}
data_10114 <- clean_data(captures = captures, surveys = surveys, translocations = translocations)
```
...
```{r write-model}
write_rds(model_10114, here::here("out", "model", "10114_model.rds"), "xz", compression = 9L)
```

- To read model file, use "model_10114 <- read_rds(here::here("out", "model", "10114_model.rds"))"
...
pars_to_plot <- c('alpha_lambda', 
                  ...
                  'sigma_phi', 
                  'beta_detect')
bayesplot::mcmc_trace(model_10114$m_fit$draws(pars_to_plot))

model_10114$m_fit$summary(pars_to_plot)
```
...
```{r plots-default}
# plot_model(model_10114, what = "abundance")
# plot_model(model_10114, what = "recruitment")
# plot_model(model_10114, what = "survival")
```
...
library(ggplot2)
p1 <- plot_model(model_10114, what = "survival") 
p1 <- p1 + ggtitle("A. Survival of translocated frogs") +
    expand_limits(y = 0) +
    theme(legend.position = "none")
  
p2 <- plot_model(model_10114, what = "abundance") 
p2 <- p2 + ggtitle("B. Population size") +
    ylab("Number of adults") + 
    expand_limits(y = 0)
  
p3 <- plot_model(model_10114, what = "recruitment")
...
```{r save-plot}
ggsave(here::here("out", "plots", "10114_mrmr_plots.png"), width = 6.5, height = 7.2)
```
...
```{r survival-tables}
survival_cohort <- survival_table(model_10114, by_cohort = TRUE)
survival_cohort

survival_ind <- survival_table(model_10114, by_individual = TRUE) 
survival_ind
```
...
survival_cohort %>% write_csv(here::here("out", "tables", "10114_survival_cohort.csv"))
survival_ind %>% write_csv(here::here("out", "tables", "10114_survival_ind.csv"))
```

### Obtain and view abudance estimate values
```{r obtain-values-for-abundance }
...
model_10114$m_fit$draws("N", format = "draws_df") %>% 
