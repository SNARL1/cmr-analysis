---
title: "11009 - Analyze capture-mark-recapture data using *mrmr*"
author: "Roland Knapp"
...
captures <- read_csv(here::here("data", "11009_capture.csv"))
surveys <- read_csv(here::here("data", "11009_survey.csv"))
```

## Check datasets
```{r check-data}
data_11009 <- clean_data(captures = captures, surveys = surveys)
```
...
Add "control = list(adapt_delta = 0.99)" or similar if divergent transitions occur. Using the "tong" server, the model-fitting process required ~30 minutes. 
```{r fit-model}
model_11009 <- fit_model(
                    data = data_11009, 
                    chains = 4, 
                    parallel_chains = 4, 
                    cpp_options = list(stan_threads = TRUE),
                    threads_per_chain = 12, 
                    dir = "/home/snarg/roland/11009") # to prevent writing to a locked system directory 
```
...
```{r write-model}
write_rds(model_11009, here::here("out", "11009_model.rds"), "xz", compression = 9L)
```
```{r read-model}
# model_11009 <- read_rds(here::here("out", "11009_model.rds"))
```
...
pars_to_plot <- c('alpha_lambda', 
                  ...
                  'sigma_phi', 
                  'beta_detect')
bayesplot::mcmc_trace(model_11009$m_fit$draws(pars_to_plot))

model_11009$m_fit$summary(pars_to_plot)
```
...
```{r plots-default}
# plot_model(model_11009, what = "abundance")
# plot_model(model_11009, what = "recruitment")
# plot_model(model_11009, what = "survival")
```

### Customize plots
```{r plots-custom, fig.height=8, fig.width=7}
library(ggplot2)
p1 <- plot_model(model_11009, what = "abundance") 
p1 <- p1 + ggtitle("A. Population size") +
    ylab("Number of adults") + 
    expand_limits(y = 0)
  
p2 <- plot_model(model_11009, what = "recruitment")
...
### Save custom plots
```{r save-plot}
ggsave(here::here("out", "11009_mrmr_plots.png"), width = 6.5, height = 7.2)
```
