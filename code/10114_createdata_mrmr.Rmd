...
where site_id in (10114, 11886, 11889, 11887) and
      survey_type = 'cmr'
```
...
- Check that non-primary sites have concurrent survey dates (or at least 'non-independent' dates).
- For 10114, modifies the code by commenting out the site specific filter, because marked/reintroduced frogs are found in neighboring sites to which they have migrated. Then, refines to unique survey dates.
...
#### Include surveys at relevant sites
...
There are four reintroduction dates for 10114 in each of 2021-2024. All frogs were released into 10114.
...
# Acceptable outcome:
# at goddard 10114, 
# the first resurvey day in 2022 found no frogs.
# the July 2024 survey had one secondary period with no frogs (July 16, the second/middle day of the trip)

# Simpler alternative, but does not discount the reintro dates.
setdiff(survey_10114$survey_date, capture_10114$visit_date) %>% as.Date(origin = "1970-01-01")
# this suggests that at Goddard 10114, three survey says turned up no frogs - but, some of those dates are reintro dates and not true surveys.
```
...
- tentatively recode all captures to occur at 10114
```{r clean-capture-data}
capture_10114 <- capture_10114 %>% 
