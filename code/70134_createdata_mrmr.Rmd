---
title: "70134 - Create datasets for analysis using *mrmr*"
author: "Roland Knapp"
...
```{sql retrieve_translocation_data, connection=con, output.var = "translocation_70134"}
select 
  release_siteid1, 
  release_date, 
  type, 
...
where release_siteid1 = 70134 and
      type = 'translocation'
```
```{r}
dbDisconnect(con)
...
translocation_70134 %>% write_csv(here::here("data", "raw", "70134_translocation_raw.csv"))
```
```{r read_raw_translocation_data, message=FALSE}
translocation_70134 <- read_csv(here::here("data", "raw", "70134_translocation_raw.csv")) %>% 
...
assert_that(noNA(translocation_70134$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters (or 9 for early sites)
assert_that(!any(nchar(translocation_70134$pit_tag_ref) != 9))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(translocation_70134$pit_tag_ref, "\\.1$")))

# Check for records of subadult frogs
translocation_70134 %>% drop_na(length) %>% filter(length < 40)

# Check for frogs that died during translocation
translocation_70134 %>% filter(grepl("dead", comment_relocate_frog) | grepl("died", comment_relocate_frog))

# Check for duplicate pit tags on each date
translocation_70134 %>% count(release_date, pit_tag_ref) %>% filter(n > 1)

# Are release dates correct?
translocation_70134 %>% distinct(release_date) %>% glimpse()
```
...
translocation_70134_mrmr <- translocation_70134 %>% 
...
translocation_70134_mrmr %>% write_csv(here::here("data", "clean", "70134_translocation.csv"))
```
```{r read_cleaned_translocation_data, message=FALSE}
translocation_70134 <- read_csv(here::here("data", "clean", "70134_translocation.csv")) %>% 
...
```{sql retrieve_survey_data, connection=con, output.var = "survey_70134"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
where (site_id = 70134 or site_id = 70688) and
      (survey_type = 'cmr' or survey_type = 'visual')
      and visit_date between '2006-08-08' and '2008-12-31'
```
```{r, message=FALSE}
dbDisconnect(con)
rm(con)
...
survey_70134 %>% write_csv(here::here("data", "raw", "70134_survey_raw.csv"))
```
```{r read_raw_survey_data, message=FALSE}
survey_70134 <- read_csv(here::here("data", "raw", "70134_survey_raw.csv"))
```

### When multiple numbered sites exist in survey area, include only one survey for entire site on each survey date
70134 is the primary site at this location and was surveyed during each secondary period. 
...
survey_70134 %>% 
  ...
  filter(n == 1 & site_id != "70134")
```
```{r include_relevant_surveys}
# Include only cmr surveys
survey_70134 <- survey_70134 %>% 
  add_count(visit_date) %>% 
  filter(n > 2 & site_id == "70134" & survey_type == "cmr" | n < 3 & site_id == "70134") %>% 
  filter((visit_date == "2006-08-22" & survey_type == "cmr") | visit_date != "2006-08-22") %>% 
  filter((visit_date == "2007-05-25" & survey_type == "cmr") | visit_date != "2007-05-25") %>% 
...
survey_70134_mrmr <- translocation_70134_mrmr %>% 
  distinct(site_id, release_date, type) %>% 
  rename(survey_type = type, survey_date = release_date) %>% bind_rows(survey_70134) %>% 
  arrange(survey_date) 
```
...
survey_70134_mrmr %>% count(site_id, survey_date) %>% filter(n > 1)

# Check for sites that were unsuitable for survey
survey_70134_mrmr %>% filter(visit_status != "suitable")
```
...
survey_70134_mrmr <- survey_70134_mrmr %>%
...
survey_70134_mrmr %>% write_csv(here::here("data", "clean", "70134_survey.csv"))
```
```{r read_cleaned_survey_data}
survey_70134 <- read_csv(here::here("data", "clean", "70134_survey.csv"))
```
...
```{sql retrieve_capture_data, connection=con, output.var = "capture_70134"}
select 
  site_id, 
  visit_date, 
  visit_status, 
...
capture_70134 %>% write_csv(here::here("data", "raw", "70134_capture_raw.csv"))
```
```{r read_raw_capture_data, message=FALSE}
capture_70134 <- read_csv(here::here("data", "raw", "70134_capture_raw.csv")) %>% 
  mutate(pit_tag_ref = as.character(pit_tag_ref))
```
...
# Check for records where pit_tag_ref is NULL
assert_that(noNA(capture_70134$pit_tag_ref))

# Check for records containing pit tags shorter than 15 characters (or 9 for early sites)
assert_that(!any(nchar(capture_70134$pit_tag_ref) != 9))

# Check for records containing pit tags that end with ".1" (known to be potentially incorrect) 
assert_that(!any(str_detect(capture_70134$pit_tag_ref, "\\.1$")))

# Check for records where species != ramu
capture_70134 %>% drop_na(species) %>% filter(species != "ramu")

# Check for records were survey_type = swab
assert_that(!any(capture_70134$survey_type == "swab"))

# Check for records where capture_animal_state is NULL
assert_that(noNA(capture_70134$capture_animal_state))

# Check for records where capture_animal_state == "dead"
assert_that(!any(capture_70134$capture_animal_state == "dead"))

# Check for records of subadult frogs
capture_70134 %>% drop_na(length) %>% filter(length < 40)

# Check for sites that were unsuitable for survey
assert_that(!any(capture_70134$visit_status != "suitable"))

# Check for duplicate capture records on each date
capture_70134 %>% count(visit_date, pit_tag_ref) %>% filter(n > 1)

# Check for captures that lack associated surveys
survey_70134 %>% 
  select(survey_date) %>% 
  rename(visit_date = survey_date) %>% 
  right_join(capture_70134, by = c("visit_date")) %>% 
...
survey_70134 %>% 
  select(survey_date) %>% 
  rename(visit_date = survey_date) %>% 
  left_join(capture_70134, by = c("visit_date")) %>% 
...
capture_70134_mrmr <- capture_70134 %>%
...
capture_70134_mrmr %>% write_csv(here::here("data", "clean", "70134_capture.csv"))
```
```{r read_cleaned_capture_data}
capture_70134 <- read_csv(here::here("data", "clean", "70134_capture.csv"))
```
