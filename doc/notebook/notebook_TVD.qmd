...
The original mrmr model provided essential population metrics as generated quantities (abundance, recruitment) but lacks easy access to other parameters of interest, including survey-specific detection probabilities, population-level survival probability estimates between primary periods, and individual log-likelihoods, which are useful for model comparison.
...
The modified model has been tested with *Rana sierrae* mark-recapture datasets from MLRG's site 54188 and from the California Department of Fish and Wildlife's Mossy Pond study. Key findings include:
